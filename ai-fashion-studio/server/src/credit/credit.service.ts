...
export class CreditService {
    ...
    private normalizeReason(reason: string | undefined, fallback: string): string {
    ...
    async hasEnoughCreditsForAmount(
        ...
        const required = Number.isFinite(amount) ? Math.max(0, Math.floor(amount)) : 0;
        const userCredits = await this.getUserCredits(userId);
        return {
            enough: userCredits.balance >= required,
            required,
    ...
    async spendCredits(
        ...
        const { newBalance } = await this.prisma.$transaction(async (tx) => {
            ...
            if (updated.count !== 1) {
                throw new BadRequestException(`积分不足。需要 ${normalized} 积分，当前余额 ${user.credits ?? 0} 积分`);
            }
        ...
        this.logger.log(`💳 用户 ${userId} 消费 ${normalized} 积分: ${reason}。余额: ${newBalance}`);
        return true;
    }
    ...
    async addCredits(
        ...
        const { newBalance } = await this.prisma.$transaction(async (tx) => {
            try {
                ...
                if (e?.code === 'P2025') throw new NotFoundException(`用户不存在: ${userId}`);
                throw e;
            }
        ...
        this.logger.log(`💰 用户 ${userId} 充值 ${normalized} 积分: ${reason}。余额: ${newBalance}`);
    }
