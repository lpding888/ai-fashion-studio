...
export class StylePresetMigrationService {
  ...
  async getMigrationStatus(): Promise<MigrationStatus> {
    ...
    const pendingPresets = allPresets.filter(
      (preset) =>
        preset.imagePaths &&
        preset.imagePaths.some((p) => !p.startsWith('http')),
    );
  ...
  async migrateToCoS(): Promise<MigrationResult> {
    ...
    this.logger.log(
      `Starting migration for ${localPresets.length} style presets...`,
    );
      ...
      try {
        ...
        for (let i = 0; i < preset.imagePaths.length; i++) {
          ...
          if (!(await fs.pathExists(normalizedPath))) {
            this.logger.warn(
              `Local file not found: ${normalizedPath}, skipping`,
            );
            continue;
          }
          ...
          this.logger.log(
            `Uploading ${preset.name} image ${i + 1}/${preset.imagePaths.length} to COS...`,
          );
        ...
        if (newImagePaths.length > 0) {
          ...
          this.logger.log(
            `✅ Migrated preset: ${preset.name} (${newImagePaths.length} images)`,
          );
          migratedCount++;
        }
      } catch (error) {
