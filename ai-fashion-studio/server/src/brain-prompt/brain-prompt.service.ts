...
export class BrainPromptService implements OnModuleInit {
    ...
    async ensureInitialized() {
        ...
        if (!seed) {
            this.logger.warn('No seed system prompt found; initialized empty prompt store');
            return;
        }

        const createdBy = { id: 'system', username: 'system' };
        const seeded = await this.createVersion(seed, createdBy, 'Seed from docs', true);
        this.logger.log(`Seeded brain prompt store with version ${seeded.versionId}`);
    }
    ...
    async getActive(): Promise<{ ref: ActivePromptRef | null; version: BrainPromptVersion | null }> {
