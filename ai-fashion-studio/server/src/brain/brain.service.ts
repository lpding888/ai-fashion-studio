...
function normalizeStyleLearnV1(input: any): z.infer<typeof StyleLearnV1Schema> {
  ...
  const lightingObj = (() => {
    ...
    const summary =
      String(v || '').trim() || 'Physically plausible studio/daylight setup.';
    return { summary };
  })();
  ...
  const cameraObj = (() => {
    ...
    if (v && typeof v === 'object' && !Array.isArray(v)) return v;
    const summary =
      String(v || '').trim() || 'Commercial fashion photo lens + DOF.';
    return { summary };
  })();
  ...
  const compositionObj = (() => {
    ...
    return {
      summary:
        'Clean fashion composition, subject-centered, readable garment silhouette.',
    };
  })();
  ...
  const sceneObj = (() => {
    ...
    if (v && typeof v === 'object' && !Array.isArray(v)) return v;
    const summary =
      String(v || '').trim() || 'Minimal commercial set / location context.';
    return { summary };
  })();
  ...
  const gradingObj = (() => {
    ...
    if (v && typeof v === 'object' && !Array.isArray(v)) return v;
    const summary =
      String(v || '').trim() || 'Neutral-to-stylized commercial color grading.';
    return { summary };
  })();
...
function normalizePoseLearnV1(input: any): z.infer<typeof PoseLearnV1Schema> {
  ...
  const poseObj = (() => {
    const v = src?.pose;
    if (v && typeof v === 'object' && !Array.isArray(v)) return v;
    const constraints = Array.isArray(src?.pose_constraints)
      ? src.pose_constraints
      : [];
  ...
  const constraints = (() => {
    ...
    if (Array.isArray(v) && v.length)
      return v.map((x) => String(x).trim()).filter(Boolean);
    const legacy = Array.isArray(src?.pose_constraints)
      ? src.pose_constraints
      : [];
...
const EcommerceShotPurposeSchema = z.preprocess(
  (value) => {
    ...
    // Chinese shortcuts
    if (v.includes('主图') || v.includes('封面') || v.includes('首图'))
      return 'main_listing';
    if (v.includes('细节') || v.includes('特写')) return 'detail';
    if (v.includes('背面') || v === '背') return 'back';
    if (v.includes('侧面') || v === '侧') return 'side';
    if (v.includes('纹理') || v.includes('材质') || v.includes('面料'))
      return 'texture';
    if (v.includes('场景') || v.includes('生活') || v.includes('氛围'))
      return 'lifestyle';
    if (v.includes('尺码') || v.includes('标签') || v.includes('洗标'))
      return 'size_tag';
    if (v.includes('包装') || v.includes('吊牌') || v.includes('外箱'))
      return 'packaging';
    ...
    if (['size_tag', 'size', 'tag', 'label', 'care_label'].includes(v))
      return 'size_tag';
...
const ShotSchema = z
  .object({
    ...
    type: z.string(),
    layout: z
      .enum(['Individual', 'Grid', 'Split', 'FilmStrip'])
      .optional()
      .default('Individual'),
...
export type HeroStoryboardPlannerOutput = z.infer<
  typeof HeroStoryboardPlannerOutputSchema
>;
...
export class BrainService {
  ...
  private async loadSystemPrompt() {
    try {
      ...
      const activeManagedPath = path.join(
        process.cwd(),
        'data',
        'brain-prompts',
        'active.md',
  ...
  private normalizeStoryboardPlanShape(parsed: any, shotCount: number) {
    ...
    if (!cards && parsed && typeof parsed === 'object') {
      ...
      const firstArrayEntry = Object.entries(parsed).find(([, v]) =>
        Array.isArray(v),
      );
    ...
    const normalizeCard = (src: any, idx: number) => {
      ...
      return {
        index: idx + 1,
        action: String(
          src?.action ?? src?.pose ?? src?.motion ?? src?.movement ?? '',
        ),
        blocking: String(
          src?.blocking ?? src?.staging ?? src?.position ?? src?.stance ?? '',
        ),
        camera: String(
          src?.camera ??
            src?.camera_move ??
        ...
        continuity: String(
          src?.continuity ?? src?.transition ?? src?.notes ?? '',
        ),
      };
    };

    const normalizedCards = (cards || [])
      .slice(0, safeShotCount)
      .map(normalizeCard);
  ...
  private normalizeHeroStoryboardPlannerOutput(
    parsed: any,
    shotCount: number,
  ): HeroStoryboardPlannerOutput {
    const safeShotCount =
      Number.isFinite(shotCount) && shotCount > 0 ? Math.floor(shotCount) : 4;

    const resolvedParams =
      parsed?.resolved_params ?? parsed?.resolvedParams ?? {};
    const visualAudit =
      parsed?.visual_audit ??
      parsed?.visualAudit ??
    ...
    const cards = this.normalizeStoryboardPlanShape(
      parsed,
      safeShotCount,
    )?.cards;
  ...
  private async encodeImageForBrain(
    filePath: string,
  ): Promise<BrainEncodedImage> {
    ...
    if (input.startsWith('http://') || input.startsWith('https://')) {
      ...
      const brainCiEnabled =
        String(process.env.BRAIN_CI_ENABLED || 'true')
          .trim()
          .toLowerCase() !== 'false';
      ...
      if (
        useDirectURL &&
        brainCiEnabled &&
        this.cosService.isValidCosUrl(input)
      ) {
        try {
          ...
          this.logger.warn(
            `Failed to generate CI URL for ${input}, fallback to original URL`,
            error,
          );
        }
      }
    ...
    if (!this.cosService.isEnabled()) {
      const allowInline =
        String(process.env.ALLOW_BRAIN_INLINE_DATA || '')
          .trim()
          .toLowerCase() === 'true' ||
      ...
      if (!allowInline) {
        throw new Error(
          'COS未配置：生产环境禁止服务器把图片转Base64发给模型；请启用COS或改为前端直传COS URL',
        );
      }
  ...
  async planTask(
    ...
    if (shouldMock) {
      ...
      return {
        plan: {
          visual_analysis: {
            category: 'Mock Hoodie',
            hero_feature: 'Mock Fabric',
            vibe: 'Streetwear',
          },
          styling_plan: {
            upper: 'Mock Top',
          ...
          shots: [
            ...
            },
            {
              ...
              type: 'Detail',
              prompt_en: 'Mock Prompt 2',
              layout: 'Individual',
            },
          ],
        },
        thinkingProcess:
          '[Mock Thinking] Analyzing garment... Planning shots...',
      };
    }
    ...
    if (!model) {
      throw new Error(
        'Brain模型未配置，请在设置页面配置Brain模型（如：gemini-3-pro-preview）',
      );
    }
    ...
    for (const imgPath of imagePaths) {
      try {
        ...
        this.logger.log(
          `🌐 Garment image (${encoded.kind}): ${encoded.kind === 'fileData' ? encoded.fileUri : '[inlineData]'}`,
        );
      } catch (e) {
    ...
    if (options.style_ref_paths && options.style_ref_paths.length > 0) {
      this.logger.log(
        `Adding ${options.style_ref_paths.length} style reference image(s)`,
      );
      for (const refPath of options.style_ref_paths) {
        try {
          ...
          contentParts.push(this.toBrainImagePart(encoded));
          this.logger.log(
            `🌐 Style ref (${encoded.kind}): ${encoded.kind === 'fileData' ? encoded.fileUri : '[inlineData]'}`,
          );
        } catch (e) {
    ...
    if (options.face_ref_paths && options.face_ref_paths.length > 0) {
      this.logger.log(
        `Adding ${options.face_ref_paths.length} face reference image(s)`,
      );
      for (const facePath of options.face_ref_paths) {
        try {
          ...
          contentParts.push(this.toBrainImagePart(encoded));
          this.logger.log(
            `🌐 Face ref (${encoded.kind}): ${encoded.kind === 'fileData' ? encoded.fileUri : '[inlineData]'}`,
          );
        } catch (e) {
    ...
    try {
      ...
      const summarizeRequest = () => {
        try {
          const contents = Array.isArray((requestBody as any)?.contents)
            ? (requestBody as any).contents
            : [];
          const parts = Array.isArray(contents?.[0]?.parts)
            ? contents[0].parts
            : [];
          const hasInline = parts.some(
          ...
          return {
            ...
            hasInline,
            hasFileData,
            responseMimeType: (requestBody as any)?.generationConfig
              ?.responseMimeType,
          };
        } catch {
          return { parts: 0 };
      ...
      this.logger.log(
        `📤 Request summary: ${JSON.stringify(summarizeRequest())}`,
      );
      ...
      if (thinkingProcess) {
        this.logger.log(
          `💭 Thinking process preview: ${thinkingProcess.substring(0, 150)}...`,
        );
      }
      ...
      this.logger.log(
        `✂️ Extracted JSON preview (first 300 chars): ${cleanContent.substring(0, 300)}`,
      );
  ...
  async planStoryboard(
    ...
    if (shouldMock) {
      ...
      const shots = Array.from({ length: options.shot_count || 4 }).map(
        (_, idx) => ({
          id: String(idx + 1).padStart(2, '0'),
          shot_type: 'mock',
          goal: 'mock goal',
      ...
      return {
        plan: {
          resolved_params: { shot_count: options.shot_count || 4 },
          shots,
        },
      };
    }
    ...
    try {
      ...
      const normalized = this.normalizeHeroStoryboardPlannerOutput(
        parsed,
        options.shot_count,
      );
  ...
  private async postWithKeyFailover(params: {
    ...
    const { keyPool, buildEndpoint, requestBody, axiosConfig, logLabel } =
      params;
...
Please output the modified English prompt that incorporates the user's requested changes:`;
    ...
    try {
      ...
      this.logger.log(
        `Fix prompt generated: ${fixPromptEn.substring(0, 100)}...`,
      );

      return { fixPromptEn: fixPromptEn.trim() };
    } catch (error: any) {
      this.logger.error(
        'Fix translation failed',
        error.response?.data || error.message,
      );
