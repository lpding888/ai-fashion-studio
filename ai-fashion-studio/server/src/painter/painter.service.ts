...
export class PainterService {
    ...
    private getFileDataFromPart(part: any): { fileUri: string; mimeType?: string } | null {
    ...
    private async downloadImageFromUrl(url: string): Promise<{ buffer: Buffer; mimeType?: string }> {
        ...
        const mimeType = String(response.headers?.['content-type'] || '').split(';')[0].trim() || undefined;
    ...
    private async callPainterGenerateContentWithRetries(args: {
    ...
    }): Promise<{ imagePath: string; shootLogText: string; shootLogTextRaw?: string }> {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (attempt > 1) {
                    this.logger.warn(`🔁 Painter retry with next key (attempt ${attempt}/${maxAttempts})`);
                }
                ...
                try {
                    ...
                    if (status < 200 || status >= 300) {
                        ...
                        const errText = await this.readStreamAsText(response.data as Readable, 64 * 1024);
                    ...
                    const handleOnePayload = async (data: any) => {
                        ...
                        for (const part of candidate.content?.parts || []) {
                            ...
                            if (!fd) continue;
                            const mimeType = fd.mimeType || this.guessMimeTypeFromPathOrHeader(fd.fileUri);
                    ...
                    if (isEventStream || isNdjson) {
                        const result = await this.readGeminiStreamForImage(response.data as Readable, async (obj) => {
                            const found = await handleOnePayload(obj);
                            return found;
                        });
                        const buffer = result.kind === 'inline'
                        ...
                        this.logger.log(`✅ Found image data, mime: ${result.mimeType}, size: ${(buffer.length / 1024).toFixed(1)}KB`);
                        this.logger.log(`💾 Image saved to ${savePath}`);
                        const raw = this.sanitizeShootLogForDump(shootLogText);
                        return { imagePath: savePath, shootLogText: this.sanitizeModelShootLogText(shootLogText), shootLogTextRaw: raw };
                    }

                    // 兜底：按 JSON 全量读取（普通 generateContent）
                    const rawText = await this.readStreamAsText(response.data as Readable, 40 * 1024 * 1024);
                    ...
                    if (!found) {
                        ...
                        this.logger.error(`❌ No image in response parts (finishReason=${finishReason || 'UNKNOWN'}, parts=${JSON.stringify(partKinds)})`);
                        const err: any = new Error(`No image data found in API response. finishReason=${finishReason || 'UNKNOWN'} ModelTextPreview=${preview || 'EMPTY'}`);
                    ...
                    const buffer = found.kind === 'inline'
                    ...
                    this.logger.log(`✅ Found image data, mime: ${found.mimeType}, size: ${(buffer.length / 1024).toFixed(1)}KB`);
                    this.logger.log(`💾 Image saved to ${savePath}`);
                    const raw = this.sanitizeShootLogForDump(shootLogText);
                    return { imagePath: savePath, shootLogText: this.sanitizeModelShootLogText(shootLogText), shootLogTextRaw: raw };
                } finally {
                ...
                if (errorData && typeof errorData === 'object' && typeof errorData.on === 'function') {
                    try {
                        safeError = await this.readStreamAsText(errorData as Readable, 32 * 1024);
                    } catch {
                        // ignore
                    }
                ...
                const wrapped = new Error(`Image generation failed: ${err?.message || String(err)}`);
                lastError = wrapped;

                const shouldRetry = attempt < maxAttempts && this.isRetryablePainterFailure(wrapped);
                if (shouldRetry) {
                    this.logger.warn(`Painter failed (attempt ${attempt}/${maxAttempts}), retrying once...`);
    ...
    private async encodeImageForPainter(
        ...
        if (input.startsWith('http://') || input.startsWith('https://')) {
            const finalUrl = options.allowCi ? this.applyTencentCiCompressionIfPossible(input).url : input;
            return {
                url: finalUrl,
        ...
        const finalUrl = options.allowCi ? this.applyTencentCiCompressionIfPossible(url).url : url;
        return {
            url: finalUrl,
            mimeType: this.guessMimeTypeFromPathOrHeader(finalUrl),
        };
    ...
    async generateImageWithLog(
        ...
        this.logger.log(`🎨 Painter Config - Gateway: ${gateway?.substring(0, 30)}..., Model: ${model || 'UNDEFINED'}, KeyPool: ${keyPool.length}`);
        ...
        const isInpaint = String(options.editMode || '').toUpperCase().includes('INPAINT');
        ...
        for (let i = 0; i < refImagePaths.length; i += 1) {
            ...
            try {
                ...
                const encoded = await this.encodeImageForPainter(imgPathOrUrl, { allowCi });
                ...
                this.logger.log(`🌐 Reference image (fileData): ${encoded.url.substring(0, 80)}...${allowCi ? '' : ' [NO_CI]'}`);
            } catch (e: any) {
                this.logger.warn(`⚠️ Failed to encode reference image: ${String(imgPathOrUrl).slice(0, 120)}`, e?.message || e);
            }
        }
        ...
        const generationConfig: any = {
            ...
            responseModalities: (Array.isArray(options.responseModalities) && options.responseModalities.length > 0)
        ...
        if (Number.isFinite(options.seed)) {
            generationConfig.seed = Math.trunc(options.seed as number);
        }
        ...
        this.logger.log(`📤 Generation Config: ${JSON.stringify(generationConfig)}`);
        ...
        try {
            ...
            const alreadyImageOnly = Array.isArray(options.responseModalities)
                && options.responseModalities.length === 1
                && options.responseModalities[0] === 'IMAGE';
            if (!alreadyImageOnly && this.isNoImageResponseError(e)) {
                const shootLogText = typeof e?.shootLogText === 'string' ? e.shootLogText : '';
                const payload2 = JSON.parse(JSON.stringify(payload));
                payload2.generationConfig = { ...(payload2.generationConfig || {}), responseModalities: ['IMAGE'] };
                ...
                if (Array.isArray(payload2?.contents) && payload2.contents.length > 0) {
                    ...
                    if (last?.role === 'user' && Array.isArray(last.parts) && last.parts.length > 0 && typeof last.parts[0]?.text === 'string') {
                ...
                return { imagePath: r2.imagePath, shootLogText: shootLogText || r2.shootLogText };
            }
            throw e;
        }
    }
    ...
    async generateImageWithChatSessionWithLog(args: {
        ...
        const gateway = args.config?.painterGateway || args.config?.gatewayUrl || "https://api.vectorengine.ai/v1";
        ...
        const sentImages: Array<{ label: string; fileUri: string; mimeType?: string; allowCi?: boolean }> = [];
        for (let i = 0; i < images.length; i += 1) {
            ...
            const allowCi = typeof img?.allowCi === 'boolean' ? img.allowCi : !(isInpaint && i < 2);
            ...
            sentImages.push({ label, fileUri: encoded.url, mimeType: encoded.mimeType, allowCi });
        }
        ...
        try {
            ...
            if (sentImages.length === 0) {
                ...
                for (const img of sentImages) {
                    lines.push(`- ${img.label}: ${img.fileUri}${img.mimeType ? ` (${img.mimeType})` : ''}${img.allowCi === false ? ' [NO_CI]' : ''}`);
                }
            }
            lines.push('');
        ...
        try {
            ...
            try {
                ...
                const dumped = await dumpPromptText({
                    ...
                    refId,
                    content: String((result as any).shootLogTextRaw || result.shootLogText || ''),
                });
                ...
                logLargeText({
                    log: (m) => this.logger.log(m),
                    header: `${header} saved=${dumped.filePath.replace(/\\\\/g, '/')} sha256=${dumped.sha256.slice(0, 12)}`,
                    text: String((result as any).shootLogTextRaw || result.shootLogText || ''),
                    chunkSize: 3200,
                    maxLen: 120_000,
                });
            } catch {
