...
export class CosService {
  ...
  constructor() {
    ...
    if (process.env.TENCENT_SECRET_ID && process.env.TENCENT_SECRET_KEY) {
      const useInternal = String(process.env.COS_USE_INTERNAL_ENDPOINT || '')
        .trim()
        .toLowerCase();
      ...
      const domainTemplate =
        (process.env.COS_UPLOAD_DOMAIN_TEMPLATE || '').trim() ||
      ...
      this.logger.log(
        `COS客户端初始化成功（uploadDomain=${domainTemplate || 'default'}）`,
      );
    } else {
  ...
  async getUploadCredentials(userId?: string) {
    if (!this.cos) {
      ...
      throw new Error(
        'COS未配置，请在.env.local中设置TENCENT_SECRET_ID和TENCENT_SECRET_KEY',
      );
    }
    ...
    if (!appId) {
      throw new Error(
        `无法从Bucket名称解析AppId: ${bucket}（期望形如 xxx-<AppId>）`,
      );
    }
