  ...
  private registerTools() {
    ...
    this.server.tool(
      'search_styles',
      {
        query: z
          .string()
          .describe("Style keywords (e.g. 'cyberpunk', 'vintage', 'rainy')"),
      },
      async ({ query }) => {
        ...
        const matches = allStyles.filter(
          (s) =>
        ...
        const topResults = matches.slice(0, 5).map((s) => ({
          ...
          analysis_summary: s.analysis
            ? `Lighting: ${s.analysis.lighting}, Vibe: ${s.analysis.vibe}`
    ...
    this.server.tool(
      ...
      async ({ style_id }) => {
        ...
        this.logger.log(
          `📖 MCP Tool performing get_style_details: "${style_id}"`,
        );
        ...
        if (!style) {
          return {
            isError: true,
            content: [
              { type: 'text', text: `Style with ID ${style_id} not found.` },
            ],
          };
        }
    ...
    this.server.tool(
      ...
      async (args) => {
        ...
        this.logger.log(
          `📝 MCP Tool proposing task creation: ${args.style_id}`,
        );
    ...
    this.logger.log(
      '✅ MCP Tools registered: search_styles, get_style_details, create_image_task',
    );
  }
}
