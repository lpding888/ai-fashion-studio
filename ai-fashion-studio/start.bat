...
:: 检查并关闭 3001 端口
set PORT_3001_FOUND=0
for /f "tokens=5" %%a in ('netstat -ano ^| findstr ":3001" ^| findstr "LISTENING" 2^>nul') do (
    set PORT_3001_FOUND=1
    echo ! 端口 3001 已被占用 (PID: %%a^)
    echo   正在关闭占用进程...
    taskkill /F /PID %%a >nul 2>&1
    if errorlevel 1 (
        echo   × 无法关闭，继续尝试启动...
    ) else (
        echo   ✓ 已关闭端口 3001 的占用进程
    )
)
if %PORT_3001_FOUND%==0 echo ✓ 端口 3001 空闲
...
:: ============ 启动服务 ============
echo [1/3] 启动后端服务器 (NestJS - Port 3001)...
start "AI Fashion Studio - Backend (3001)" cmd /k "cd server && npm run start:dev"
