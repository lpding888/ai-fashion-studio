...
import { ActiveRef, PromptVersion, PromptVersionMeta, WorkflowPromptPack } from './types';
...
export default function WorkflowPromptsPage() {
  ...
  } = useSWR<{ ref: ActiveRef; version: PromptVersion }>('/admin/workflow-prompts/active', fetcher);
  ...
  } = useSWR<{ versions: PromptVersionMeta[] }>('/admin/workflow-prompts/versions', fetcher);
  ...
  React.useEffect(() => {
    if (activeData?.version?.pack && !plannerSystemPrompt && !painterSystemPrompt) {
      setPlannerSystemPrompt(activeData.version.pack.plannerSystemPrompt || '');
      setPainterSystemPrompt(activeData.version.pack.painterSystemPrompt || '');
    }
  ...
  const loadVersionToEditor = async (id: string) => {
    ...
    setSuccessMsg(null);
    try {
      const res = await api.get(`/admin/workflow-prompts/versions/${id}`);
  ...
  const createVersion = async (publish: boolean) => {
    ...
    try {
      const res = await api.post('/admin/workflow-prompts/versions', {
        pack: { plannerSystemPrompt, painterSystemPrompt } as WorkflowPromptPack,
        note,
        publish
      });
  ...
  const publishVersion = async (versionId: string) => {
    ...
    setActionLoading(true);
    try {
      await api.post('/admin/workflow-prompts/publish', { versionId });
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <PageHeader
          title="工作流提示词"
          description="管理 Hero 母版与分镜规划的提示词版本"
        />
      ...
      <Editor
        ...
        painterSystemPrompt={painterSystemPrompt}
        loading={actionLoading}
        activeRef={activeData?.ref || null}
        onNoteChange={setNote}
        onPlannerPromptChange={setPlannerSystemPrompt}
        onPainterPromptChange={setPainterSystemPrompt}
