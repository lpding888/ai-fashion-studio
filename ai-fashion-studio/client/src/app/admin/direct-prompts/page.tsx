...
import { ActiveRef, PromptVersion, PromptVersionMeta, DirectPromptPack } from './types';
...
export default function DirectPromptsPage() {
  ...
  } = useSWR<{ ref: ActiveRef; version: PromptVersion }>('/admin/direct-prompts/active', fetcher);
  ...
  } = useSWR<{ versions: PromptVersionMeta[] }>('/admin/direct-prompts/versions', fetcher);
  ...
  React.useEffect(() => {
    if (activeData?.version?.pack?.directSystemPrompt && !directSystemPrompt) {
      setDirectSystemPrompt(activeData.version.pack.directSystemPrompt);
    }
  ...
  const loadVersionToEditor = async (id: string) => {
    ...
    setSuccessMsg(null);
    try {
      const res = await api.get(`/admin/direct-prompts/versions/${id}`);
  ...
  const createVersion = async (publish: boolean) => {
    ...
    try {
      const res = await api.post('/admin/direct-prompts/versions', {
        pack: { directSystemPrompt } as DirectPromptPack,
        note,
        publish
      });
  ...
  const publishVersion = async (versionId: string) => {
    ...
    setActionLoading(true);
    try {
      await api.post('/admin/direct-prompts/publish', { versionId });
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <PageHeader
          title="直出图系统提示词"
          description="管理 /learn 直出图（Painter）system prompt 的版本与发布"
        />
      ...
      <Editor
        ...
        onPromptChange={setDirectSystemPrompt}
