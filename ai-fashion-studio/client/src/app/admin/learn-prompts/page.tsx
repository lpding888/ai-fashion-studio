...
import { ActiveRef, PromptVersion, PromptVersionMeta, LearnPromptPack } from './types';
...
export default function LearnPromptsPage() {
  ...
  } = useSWR<{ ref: ActiveRef; version: PromptVersion }>('/admin/learn-prompts/active', fetcher);
  ...
  } = useSWR<{ versions: PromptVersionMeta[] }>('/admin/learn-prompts/versions', fetcher);
  ...
  React.useEffect(() => {
    if (activeData?.version?.pack && !styleLearnPrompt && !poseLearnPrompt) {
      setStyleLearnPrompt(activeData.version.pack.styleLearnPrompt || '');
      setPoseLearnPrompt(activeData.version.pack.poseLearnPrompt || '');
    }
  ...
  const loadVersionToEditor = async (id: string) => {
    ...
    setSuccessMsg(null);
    try {
      const res = await api.get(`/admin/learn-prompts/versions/${id}`);
  ...
  const createVersion = async (publish: boolean) => {
    ...
    try {
      const res = await api.post('/admin/learn-prompts/versions', {
        pack: { styleLearnPrompt, poseLearnPrompt } as LearnPromptPack,
        note,
        publish
      });
  ...
  const publishVersion = async (versionId: string) => {
    ...
    setActionLoading(true);
    try {
      await api.post('/admin/learn-prompts/publish', { versionId });
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <PageHeader
          title="学习提示词"
          description="管理风格学习与动作（姿势）学习的系统提示词版本"
        />
      ...
      <Editor
        note={note}
        styleLearnPrompt={styleLearnPrompt}
        poseLearnPrompt={poseLearnPrompt}
        loading={actionLoading}
        activeRef={activeData?.ref || null}
        onNoteChange={setNote}
        onStylePromptChange={setStyleLearnPrompt}
        onPosePromptChange={setPoseLearnPrompt}
